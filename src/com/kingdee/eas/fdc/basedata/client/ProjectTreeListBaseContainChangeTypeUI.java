 ...
 * output class name
 */
public abstract class ProjectTreeListBaseContainChangeTypeUI extends AbstractProjectTreeListBaseContainChangeTypeUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProjectTreeListBaseContainChangeTypeUI.class);
    protected Set authorizedOrgs = null;
    
    public ProjectTreeListBaseContainChangeTypeUI() throws Exception
    {
        super();
    }
...
//		}
		...
		//add by ypf on 2015年6月27日09:35:10 把变更类型从listui传到editui
		Object changeTypeObj = getChangeTypeSelectedTreeNode().getUserObject();
		if(!changeTypeObj.equals("所有类型")){
			ChangeTypeCollection changeTypeCollection;
			try {
				changeTypeCollection = ChangeTypeFactory.getRemoteInstance().getChangeTypeCollection(" where name='"+changeTypeObj+"'");
				if (changeTypeCollection != null) {
					BOSUuid id = changeTypeCollection.get(0).getId();
					String number = changeTypeCollection.get(0).getNumber();
					uiContext.put("chngTypId", id);
					uiContext.put("chngTypNum", number);
				}
		...
		{
			uiContext.put("chngTypId", null);
			uiContext.put("chngTypNum", null);
		}
	}
	...
	public void onLoad() throws Exception {
		...
		buildProjectTree();//初始化工程项目树
		buildChangeTypeTree();//初始化变更类型树，add by ypf
		
		/*TreeSelectionListener conTypeTreeSelectionListener = null;
		TreeSelectionListener[] listeners2 = treeChangeType.getTreeSelectionListeners();
		if (listeners2.length > 0) {
			conTypeTreeSelectionListener = listeners2[0];
//			treeChangeType.removeTreeSelectionListener(conTypeTreeSelectionListener);
	    }*/
		...
		treeChangeType.setSelectionRow(0);
		treeChangeType.expandRow(0);
...
//		super.treeProject_valueChanged(e);
	...
	//蛋痛，没用
	protected void treeChangeType_valueChanged(TreeSelectionEvent e)throws Exception {
		super.treeChangeType_valueChanged(e);
		treeSelectChange();
	...
	@Override
	protected void treeChangeType_mouseClicked(MouseEvent e) throws Exception {
		super.treeChangeType_mouseClicked(e);
		treeSelectChange();
	...
	protected FilterInfo getTreeFilter() throws Exception{
		...
		if (getProjSelectedTreeNode() != null && getProjSelectedTreeNode().getUserObject() instanceof CoreBaseInfo) 
		{
			...
			if (projTreeNodeInfo instanceof CurProjectInfo) {
				...
				filterItems.add(new FilterItemInfo("curProject.id", idSet.toString(),CompareType.INCLUDE));
			}

		}
		
		//变更类型树
		if (getChangeTypeSelectedTreeNode() != null) 
		{
			Object changeTypeObj = getChangeTypeSelectedTreeNode().getUserObject();
			if(!changeTypeObj.equals("所有类型")){
				ChangeTypeCollection changeTypeCollection = ChangeTypeFactory.getRemoteInstance().getChangeTypeCollection(" where name='"+changeTypeObj+"'");
				if (changeTypeCollection != null) {
					BOSUuid id = changeTypeCollection.get(0).getId();
					filterItems.add(new FilterItemInfo("auditType.id", id,CompareType.EQUALS));
				}
			}else{
				//不过滤，查所有变更类型的
			}
	...
	public DefaultKingdeeTreeNode getChangeTypeSelectedTreeNode() {
		return (DefaultKingdeeTreeNode) treeChangeType.getLastSelectedPathComponent();
	}
...
//			idSet.add(projectInfo.getId().toString());
	 ...
	 * 构造合同类型树
	 */
	protected void buildChangeTypeTree() throws Exception {
		KDTree treeMain = getChangeTypeTree();
...
//			treeMain.removeTreeSelectionListener(treeSelectionListener);
		...
		DefaultKingdeeTreeNode root = new DefaultKingdeeTreeNode("所有类型");//根节点
		treeMain.setModel(new KingdeeTreeModel(root));
		
		//根据查询的变更类型集合，设置二级节点
		ChangeTypeCollection changeTypeCollection = ChangeTypeFactory.getRemoteInstance().getChangeTypeCollection();//子项目
		for (int j = 0; j < changeTypeCollection.size(); j++) {
			ChangeTypeInfo changeTypeInfo = changeTypeCollection.get(j);
			DefaultKingdeeTreeNode leaf = new DefaultKingdeeTreeNode(changeTypeInfo.getName());
    		treeMain.addNodeInto(leaf, root);
		}
...
//		treeMain.addTreeSelectionListener(treeSelectionListener);
	...
	private KDTree getChangeTypeTree() {
		return treeChangeType;
	}
